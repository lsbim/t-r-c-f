...
const ImageFilterComponent = () => {
  ...
  useEffect(() => {
    ...
    img.onload = async () => {
      ...
      c5.width = cutW;
      c5.height = cutH;
      c5.getContext('2d', { willReadFrequently: true }).drawImage(
        img,
        cutX,
        cutY,
        cutW,
  ...
  return (
    <div className="w-[1024] flex flex-col justify-center items-center gap-y-2 mb-4">
      <div className="font-bold">이진화는 경계가 사라져 OCR 오인식이 잦음</div>
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {steps.map((dataUrl, i) => (
            <div key={i} className="text-center">
              ...
              <img src={dataUrl} alt={labels[i]} className="border rounded" />
            </div>
          ))}
        </div>
      )}
